<?php

	//VERIFICACIÓN  DE DATOS
	//include ("manejadora.php");
	
	//ID DE CLIENTEs-------------> 2
	//AQUI INICIAMOS SESION 
	session_start();

//SI EL ROL NO EXISTE ENTONCES REDIRIGIR AL FORMULARIO DE INICAR SESION
	if(!isset($_SESSION['rol']))
	{
		header("location: ../iniciarSesion.php");
	}

?>

<!DOCTYPE html>
<html lang="es">
<head>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
	<title>MODIFICAR PERSONA</title>

	<!-- Normalize V8.0.1 -->
	<link rel="stylesheet" href="./css/normalize.css">

	<!-- para validaciones en tiempo real -->
	<link rel="stylesheet"href="./css/materialize.min.css"> 

	<!-- Bootstrap V4.3 -->
	<link rel="stylesheet" href="./css/bootstrap.min.css">

	<!-- Bootstrap Material Design V4.0 -->
	<link rel="stylesheet" href="./css/bootstrap-material-design.min.css">

	<!-- Font Awesome V5.9.0 -->
	<link rel="stylesheet" href="./css/all.css"> <!-- este código tiene demasiadas líneas-->

	<!-- Sweet Alerts V8.13.0 CSS file -->
	<link rel="stylesheet" href="./css/sweetalert2.min.css">

	<!-- Sweet Alert V8.13.0 JS file-->
	<script src="./js/sweetalert2.min.js" ></script>

	<!-- jQuery Custom Content Scroller V3.1.5 -->
	<link rel="stylesheet" href="./css/jquery.mCustomScrollbar.css">
	
	<!-- General Styles -->
	<link rel="stylesheet" href="./css/style.css">>

	<script src="https://cdn.jsdelivr.net/npm/sweetalert2@9"></script>
	


</head>
<body>
	
	<!-- Main container -->
	<main class="full-box main-container">
		<!-- Nav lateral -->
		<section class="full-box nav-lateral">
			<div class="full-box nav-lateral-bg show-nav-lateral"></div>
			<div class="full-box nav-lateral-content">
				<figure class="full-box nav-lateral-avatar">
					<i class="far fa-times-circle show-nav-lateral"></i>
					<img src="./assets/avatar/Avatar.png" class="img-fluid" alt="Avatar">
					<figcaption class="roboto-medium text-center">
						<?php

							echo $_SESSION['rol'];
							
							if($_SESSION['rol'] != "Administrador")
							{
								echo "<br>";
						        echo "Departamento: ".$_SESSION['departamento_usuario'];
							}


						?> 
						<br><small class="roboto-condensed-light">IUT DR. FEDERICO RIVERO PALACIOS</small>
					</figcaption>
				</figure>
				<div class="full-box nav-lateral-bar"></div>
				<nav class="full-box nav-lateral-menu">

					<!--type="hidden"  para ocultar el contendio del boton-->
					<input type="hidden" name="rol" id="rol" value="<?php echo $_SESSION['rol'] ?>">

					<ul>
						<li>
							<a href="inicio.php"><i class="fab fa-dashcube fa-fw"></i> &nbsp; Menú</a>
						</li>

						<li id="usuario"  style = "display: none" >
							<a href="#" class="nav-btn-submenu"><i class="fas fa-users fa-fw"></i> &nbsp; Gestión de usuario <i class="fas fa-chevron-down"></i></a>
							<ul>
								<li>
									<a href="./vistaCrearUsuario.php"><i class="fas fa-plus fa-fw"></i> &nbsp; Crear perfil Usuario</a>
								</li>
								<li>
									<a href="./vistaModificarUsuario.php"><i class="fas fa-clipboard-list fa-fw"></i> &nbsp; Modificar perfil usuario</a>
								</li>
								<li>
									<a href="./vistaConsultarUsuario.php"><i class="fas fa-search fa-fw"></i> &nbsp; Consultar Usuario</a>
								</li>
	
							</ul>
						</li>

						<li id="persona"  style = "display: none" >
							<a href="#" class="nav-btn-submenu"><i class="fas fa-user fa-fw"></i> &nbsp; Gestión de persona <i class="fas fa-chevron-down"></i></a>
							<ul>
								<li>
									<a href="./vistaCrearPersona.php"><i class="fas fa-plus fa-fw"></i> &nbsp; Crear perfil persona</a>
								</li>
								<li>
									<a href="./vistaModificarPersona.php"><i class="fas fa-clipboard-list fa-fw"></i> &nbsp; Modificar perfil persona</a>
								</li>
								<li>
									<a href="./vistaConsultarPersona.php"><i class="fas fa-search fa-fw"></i> &nbsp; Consultar persona</a>
								</li>
							</ul>
						</li>

						<!--SUB MODULO-->
						
						<li id="solicitud"  style="display: none" >
							<a href="#" class="nav-btn-submenu"><i class="fas fa-file-invoice fa-fw"></i> &nbsp; Gestión de Solicitudes <i class="fas fa-chevron-down"></i></a>
							<ul>
								
								<li>
									<a href="#" class="nav-btn-submenu"><i class="fas fa-file-invoice fa-fw"></i> &nbsp; Solicitud de Servicio <i class="fas fa-chevron-down"></i></a>
									<ul>
										<li>
											<a href="./vistaCrearServicio.php"><i class="fas fa-plus fa-fw"></i> &nbsp; Crear Solicitud</a>
										</li>
										<li>
											<a href="./vistaModificarServicio.php"><i class="fas fa-clipboard-list fa-fw"></i> &nbsp; Modificar Solicitud</a>
										</li>
										<li>
											<a href="./vistaConsultarServicio.php"><i class="fas fa-search fa-fw"></i> &nbsp; Consultar Solicitud</a>
										</li>
									</ul>
								</li>


								<li>
									<a href="#" class="nav-btn-submenu"><i class="fas fa-file-invoice fa-fw"></i> &nbsp; Solicitud de Refrigerio <i class="fas fa-chevron-down"></i></a>
									<ul>
										<li>
											<a href="./vistaCrearRefrigerio.php"><i class="fas fa-plus fa-fw"></i> &nbsp; Crear Solicitud</a>
										</li>
										<li>
											<a href="./vistaModificarRefrigerio.php"><i class="fas fa-clipboard-list fa-fw"></i> &nbsp; Modificar Solicitud</a>
										</li>
										<li>
											<a href="./vistaConsultarRefrigerio.php"><i class="fas fa-search fa-fw"></i> &nbsp; Consultar Solicitud</a>
										</li>
									</ul>
								</li>
							</ul>
						</li>

						<li id="memo"  style = "display: none" >
							<a href="#" class="nav-btn-submenu"><i class="fas fa-file-invoice fa-fw"></i> &nbsp; Gestión de memos <i class="fas fa-chevron-down"></i></a>
							<ul>
								<li>
									<a href="./vistaCrearMemo.php"><i class="fas fa-plus fa-fw"></i> &nbsp; Crear memo</a>
								</li>
								<li>
									<a href="./vistaModificarMemo.php"><i class="fas fa-clipboard-list fa-fw"></i> &nbsp; Modificar memo</a>
								</li>
								<li>
									<a href="./vistaConsultarMemo.php"><i class="fas fa-search fa-fw"></i> &nbsp; Consultar memo</a>
								</li>
							</ul>
						</li>

						<li id="departamento"  style = "display: none" >
							<a href="#" class="nav-btn-submenu"><i class="fas fa-store-alt fa-fw"></i> &nbsp; Gestión de departamento <i class="fas fa-chevron-down"></i></a>
							<ul>
								<li>
									<a href="./vistaCrearDepartamento.php"><i class="fas fa-plus fa-fw"></i> &nbsp; Crear departamento</a>
								</li>
								<li>
									<a href="./vistaModificarDepartamento.php"><i class="fas fa-clipboard-list fa-fw"></i> &nbsp; Modificar departamento</a>
								</li>
								<li>
									<a href="./vistaConsultarDepartamento.php"><i class="fas fa-search fa-fw"></i> &nbsp; Consultar departamento</a>
								</li>
							</ul>
						</li>
					
						<li id="correspondencia"  style="display: none">
						    <a href="#" class="nav-btn-submenu"><i class="fas fa-envelope-open-text fa-fw"></i> &nbsp; Envio Correspondencia <i class="fas fa-chevron-down"></i></a>
                        	<ul>
                            	<li>
                                	<a href="./vistaEnvioCorrespondencia.php"><i class="fas fa-plus fa-fw"></i> &nbsp; Enviar Correspondencia</a>
                            	</li>
                            	<li>
                                	<a href="./vistaCorrespondenciaEnviada.php"><i class="fas fa-clipboard-list fa-fw"></i> &nbsp; Enviada</a>
                            	</li>
                            	<li>
                               		<a href="./vistaCorrespondenciaRecibida.php"><i class="fas fa-search fa-fw"></i> &nbsp; Recibida</a>
                            	</li>
                        	</ul>
						</li>

						<li id="estatus"  style="display: none">
							<a href="../vista/vistaEstatusSolicitud.php"><i class="fas fa-file-import fa-fw"></i> &nbsp; Estatus de solicitud</a>
						</li>


						<li id="cambioImagen" style="display: none" >
							<a href="../vista/vistaMembrete.php"><i class="fas fa-file-import fa-fw"></i> &nbsp; Membrete</a>
						</li>

					</ul>
				</nav>
			</div>
		</section>



		<!-- Page content -->
			<section class="full-box page-content">
			<nav class="full-box navbar-info">
				<a href="#" class="float-left show-nav-lateral">
					<i class="fas fa-exchange-alt"></i>
				</a>
				<a href="#" class="btn-exit-system">
					<i class="fas fa-power-off"></i>
				</a>
			</nav>

			<!-- Page header -->
			<div class="full-box page-header">
				<h3 class="text-left">
					<i class="fab fa-dashcube fa-fw"></i> &nbsp; MODIFICAR PERSONA
				</h3>
				<p class="text-justify">
					Permite modificar los datos de la persona solicitada.

				</p>
			</div>
			
			<!-- Content -->
			<?php
		include('../modelo/persona.php');
		include('../modelo/telefono.php');
		include('../modelo/correo.php');
		include_once ('../modelo/claseDepartamento.php');
		$depa = $_POST['departamento'];
		$departamento=new departamento(null,$depa); //instanciamos
		$depconsulta=$departamento->consultarNombreDepartamento();
		$iddep= $depconsulta['1'];
		$p=new persona("","","","","","");
		$t=new telefono("","","");
		$c=new correo("","","");
		if($p->valPer($_POST['Cédula'],$iddep)==0){
			echo"
			<script type='text/javascript'>
			Swal.fire({
				title: '¡ERROR!',
				html: 'La cédula ingresada no esta registrada en el sistema',
				icon: 'error',
				confirmButtonText: 'VOLVER',
				allowOutsideClick: false,
				allowEscapeKey: false,
				allowEnterKey: false
			})
			.then(resultado => {
				if(resultado.value) {
					window.location.href='../vista/vistaModificarPersona.php';
				}
			});
			
			</script>
			";

		}
		if($p->valPer($_POST['Cédula'],$iddep)!=0){
			if($_POST['Modificar']==1){
				$p->Listado($_POST['Cédula']);
			}
			if($_POST['Modificar']==2){
				$t->listarTlfn($_POST['Cédula']);
				$c->listarCorreo($_POST['Cédula']);
			}
		}
		


   
	?>


	<?php if($_POST['Modificar']==1 && $p->valPer($_POST['Cédula'],$iddep)==1): ?>
	<div class='container-fluid'>
		<h5 align="center">Seleccione datos a modificar</h5>
		<table class='table table-dark table-sm'>
			<tr>
				<td align='center' width=400><input type="checkbox" id="checkNom" onchange="mostrarNom()"> Nombre</td>
				<td align='center' width=400><input type="checkbox" id="checkApe" onchange="mostrarApe()"> Apellido</td>
				<td align='center' width=400><input type="checkbox" id="checkDir" onchange="mostrarDir()"> Dirección </td>
				<td align='center' width=400><input type="checkbox" id="checkCargo" onchange="mostrarCargo()"> Cargo </td>
			</tr>
		</table>
	</div>
	<div align="center">
	<span class="helper-text" id="vSP"></span>
	</div>
	
	<div class="container-fluid">
    <form action="../controlador/controladorModificarPersona.php" method="POST" class="form-neon" autocomplete="off" id="FormPersonal" name="FormPersonal">
		
	<?php echo "<input type='hidden' name='Cédula' id='conCed' value='".$_POST['Cédula']."'>";?>
	<?php echo "<input type='hidden' name='Modificar' id='optMod' value='".$_POST['Modificar']."'>";?>
	
            
                <legend><i class="fas fa-user-lock"></i> &nbsp; Datos Personales</legend>
                <div class="row">
                    <div class="col-12 col-md-6" id="modNom" style="display: none">
                        <div class="form-group">
                            <label for="Nom" class="bmd-label-floating">Nombres</label>
                            <input type="text" class="form-control" name="Nombre" id="Nom" maxlength="35" onfocusout="validarNom()">
							<span class="helper-text"></span>
                        </div>
                    </div>
                    
                    <div class="col-12 col-md-6" id="modApe" style="display: none">
                        <div class="form-group">
                            <label for="Ape" class="bmd-label-floating">Apellidos</label>
                            <input type="text" class="form-control" name="Apellido" id="Ape" maxlength="35" onfocusout="validarApe()">
							<span class="helper-text"></span>
                        </div>
                    </div>
                </div>    
                <div class="row">

                    <div class="col-12 col-md-6" id="modDir" style="display: none">
                        <div class="form-group">
                            <label for="Dir" class="bmd-label-floating">Dirección</label>
                            <input type="text" class="form-control" name="Dirección"id="Dir"maxlength="70" onfocusout="validarDir()">
							<span class="helper-text"></span>
                        </div>
                    </div>
            
            
                    <div class="col-12 col-md-6" id="modCargo" style="display: none">
							<select class="form-control" name="Cargo" id="Cargo" onfocusout="validarCargo()">
								<option value="0" selected="true">Seleccione un cargo</option>
								<option value="1">Personal Administrativo</option>
								<option value="2">Personal Docente</option>
								<option value="3">Personal Obrero</option>
								<option value="4">Becario</option>
							</select>
							<span class="helper-text" id="tCargo"></span>
                     </div>
                </div>
              
    
    
        
        <p class="text-center" style="margin-top: 40px;">
            <button type="reset" class="btn btn-raised btn-secondary btn-sm"><i class="fas fa-paint-roller"></i> &nbsp; LIMPIAR</button>
            &nbsp; &nbsp;
            <button type="submit" class="btn btn-raised btn-info btn-sm"><i class="far fa-save"></i> &nbsp; GUARDAR</button>
		</p>
		
    </form>
</div>
<?php endif; ?>	
<?php if($_POST['Modificar']==2 && $p->valPer($_POST['Cédula'],$iddep)==1): ?>

	<div class='container-fluid'>
		<h5 align="center">Seleccione datos a modificar</h5>
		<table class='table table-dark table-sm'>
			<tr>
				<td align='center' width=400><input type="checkbox" id="checkTel" onchange="mostrarTel()"> Teléfono</td>
				<td align='center' width=400><input type="checkbox" id="checkCor" onchange="mostrarCor()"> Correo</td>
			</tr>
		</table>
	</div>
	<div align="center">
	<span class="helper-text" id="vSC"></span>
	</div>
	<div class='container-fluid'>
		<table class='table table-dark table-sm'>
		<div class="row">
                    <div class="col-12 col-md-6" id="optTel" style="display: none">
						<select class="form-control" name="selTel" id="selTel" onchange="mostrarAddDelTel()">
								<option value="0" selected="true">Seleccione una opción para Teléfono</option>
								<option value="1">Agregar</option>
								<option value="2">Eliminar</option>
							</select>
							<span class="helper-text" id="tTel"></span>
                     </div>
                     <div class="col-12 col-md-6" id="optCor" style="display: none">
					 		<select class="form-control"name="selCor" id="selCor" onchange="mostrarAddDelCor()">
								<option value="0" selected="">Seleccione una opción para Correo</option>
								<option value="1">Agregar</option>
								<option value="2">Eliminar</option>
							</select>
							<span class="helper-text" id="tCor"></span>
                        <br>
                    </div>
                </div>
		</table>
	</div>
	
	
	<div class="container-fluid">
    <form action="../controlador/controladorModificarPersona.php" method="POST" class="form-neon" autocomplete="off" id="FormContacto" name="FormContacto">
	<?php echo "<input type='hidden' name='Cédula' id='conCed' value='".$_POST['Cédula']."'>";?>
	<?php echo "<input type='hidden' name='Modificar' id='optMod' value='".$_POST['Modificar']."'>";?>
	<legend><i class="fas fa-user-lock"></i> &nbsp; Datos de Contacto</legend>
                <div class="row">	
				<div class="col-12 col-md-6" id="addNum" style="display: none">
                        <div class="form-group">
                            <label for="Tel" class="bmd-label-floating">Teléfono</label>
                            <input type="text" class="form-control" name="AgregarTeléfono" id="addTel" maxlength="35" onfocusout="validarAddTel()">
							<span class="helper-text"></span>
                        </div>
                    </div>   
                    <div class="col-12 col-md-6">
                        <div class="form-group" id="addCor" style="display: none">
                            <label for="Cor" class="bmd-label-floating">Correo</label>
                            <input type="text" class="form-control" name="AgregarCorreo" id="addCorreo" maxlength="35" onfocusout="validarAddCor()">
							<span class="helper-text"></span>
                        </div>
                    </div>
                </div>
                <div class="row">
                        <div class="col-12 col-md-6" id="addTT" style="display: none">
                            <select class="form-control" name="addTT" id="TT" onfocusout="validarAddTTel()">
                                <option value="0" selected="" >Tipo telefono</option >
                                <option value="1">Celular</option>
                                <option value="2">Local</option>
                            </select>
							<span class="helper-text" id="tTT"></span>
                        </div>
						<div class="col-12 col-md-6" id="addTC" style="display: none">
                        <select class="form-control" name="addTC" id="TC" onfocusout="validarAddTCor()">
                            <option value="0" selected="" >Tipo Correo</option>
                            <option value="1">Personal</option>
                            <option value="2">Institucional</option>
                        </select>
						<span class="helper-text" id="tTC"></span>
                    </div>
                </div>
				<div class="row">	
				<div class="col-12 col-md-6" >
                        <div class="form-group" id="delNum" style="display: none">
                            <label for="Tel" class="bmd-label-floating">Teléfono</label>
                            <input type="text" class="form-control" name="EliminarTeléfono" id="delTel" maxlength="35" onfocusout="validarDelTel()">
							<span class="helper-text"></span>
                        </div>
                 
                        <div class="form-group"  id="delCor" style="display: none">
                            <label for="Cor" class="bmd-label-floating">Correo</label>
                            <input type="text" class="form-control" name="EliminarCorreo" id="delCorreo" maxlength="35" onfocusout="validarDelCor()">
							<span class="helper-text"></span>
                        </div>
                    </div>
                </div>
              
    
    
        
        <p class="text-center" style="margin-top: 40px;">
            <button type="reset" class="btn btn-raised btn-secondary btn-sm"><i class="fas fa-paint-roller"></i> &nbsp; LIMPIAR</button>
            &nbsp; &nbsp;
            <button type="submit" class="btn btn-raised btn-info btn-sm"><i class="far fa-save"></i> &nbsp; GUARDAR</button>
		</p>
		
    </form>
</div>
<?php endif; ?>		
		
		</section>
	</main>
	
	
	<!--=============================================
	=            Include JavaScript files           =
	==============================================-->
	<!-- jQuery V3.4.1 -->
	<script src="./js/jquery-3.4.1.min.js" ></script>

	<!-- popper -->
	<script src="./js/popper.min.js" ></script>

	<!-- Bootstrap V4.3 -->
	<script src="./js/bootstrap.min.js" ></script>

	<!-- jQuery Custom Content Scroller V3.1.5 -->
	<script src="./js/jquery.mCustomScrollbar.concat.min.js" ></script>

	<!-- Bootstrap Material Design V4.0 -->
	<script src="./js/bootstrap-material-design.min.js" ></script>
	<script>$(document).ready(function() { $('body').bootstrapMaterialDesign(); });</script>
	
	
	<!-- validar menú dependiendo del rol--->
	<script src="./js/validacionRol.js" ></script>

	<script src="./js/main.js" ></script>

	<script src="./js/controladorModificarPersona.js">
	</script>


	
</body>
</html>