...
<body>
	...
	<main class="full-box main-container">
		...
		<section class="full-box page-content">
			...
			<div class="container-fluid">
        		<form class="form-neon" action="../controlador/controladorConsultarMemo.php" method="POST" id="form" name="form" >
            		<div class="container-fluid">

						<div align="center" class="form-group">
							<select class="form-control" name="optMemo" id="optMemo" onchange="mostrarOptMemo()">
								<option value="0" disabled="" selected="">Seleccione una opción</option>
								<option value="1">CORRELATIVO MEMO</option>
								<option value="2">FECHA</option>
								<option value="3">ASUNTO</option>
								<option value="4">DEPARTAMENTO AL QUE SE ENVIO</option>
               			...
               			<div class="row justify-content-md-center">
							...
							?>
                        		...
                        		<div class="form-group" id="inpID" style="display: none">
									<select class="form-control" id="idMemo" name="ID">
                                    	<option value="0" disabled=""  selected=""> SELECCIONE CORRELATIVO</option>
                        		...
                        		<div class="form-group" id="inpFecha" style="display: none">
									...
									<!--<option value="1"> Administración </option>-->
										<?php
											...
											$resultadoF=$memo->consultarMemoDepartamento();
											$row=pg_fetch_array($resultadoF);

											echo "<option value=".$row['fechad'].">".$row["fechad"]."</option>";

                                        	while($row1=pg_fetch_array($resultadoF)){
												if($row1["fechad"] != $row["fechad"]){
													echo "<option value=".$row1['fechad'].">".$row1["fechad"]."</option>";
												}
								...
								<div class="form-group" id="inpAsunto" style="display: none">
									<select class="form-control" id="asunto" name="asunto" >
                                    	<option value="0" disabled=""  selected=""> SELECCIONE ASUNTO</option>
									<!--<option value="1"> Administración </option>-->
										<?php

											$resultadoA=$memo->consultarMemoDepartamento();
                                        	$row=pg_fetch_array($resultadoA);

											echo "<option value=".$row['asunto'].">".$row["asunto"]."</option>";

                                        	while($row1=pg_fetch_array($resultadoA)){
												if($row1["asunto"] != $row["asunto"]){
													echo "<option value=".$row1['asunto'].">".$row1["asunto"]."</option>";
												}
								...
								<div class="form-group" id="inpDepa" style="display: none">
									<select class="form-control" id="nombreDepartamento" name="departamento">
						   ...
						   <input type="hidden" class="form-control" name="Consultar" id="Consultar"  value="Consultar">


                    	<div class="col-12">
                       		<p class="text-center" style="margin-top: 40px;">
                            	<button type="submit" class="btn btn-raised btn-info" name="consultarDepartamento" id="consultarDepartamento"><i class="fas fa-search" ></i> &nbsp; CONSULTAR</button>
                       		 </p>
                   		</div>
               		</div>
        		</form>
	...
	<script src="./js/vistaConsultarMemo.js" ></script>
