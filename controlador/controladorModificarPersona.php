		...
		<!-- Page content -->
			...
			<?php
                ...
                if($_POST['Modificar'] == "1"){
                    if($_POST['Nombre'] != ""){
                        $p->updateNombre($_POST['Cédula'],strtoupper($_POST['Nombre']));
                    
                    }
                    if($_POST['Apellido'] != ""){
                        $p->updateApe($_POST['Cédula'],strtoupper($_POST['Apellido']));
                    }
                    
                    
                    if($_POST['Dirección'] != ""){
                        $p->updateDir($_POST['Cédula'],strtoupper($_POST['Dirección']));
                    }
                    ...
                    if($_POST['Cargo'] != 0){
                        $p->updateCargo($_POST['Cédula'],strtoupper($_POST['Cargo']));
					}
					echo"
                ...
                if($_POST['Modificar'] == "2"){
                    ...
                    if($_POST['AgregarCorreo'] != ""){
						if($c->valCor(strtoupper($_POST['AgregarCorreo']))==1){
							echo"
							<script type='text/javascript'>
							Swal.fire({
								title: '¡ERROR!',
								html: 'Este correo ya esta registrado y no puede ser duplicado',
								icon: 'error',
								confirmButtonText: 'VOLVER',
								allowOutsideClick: false,
								allowEscapeKey: false,
							...
							$c->addCor(strtoupper($_POST['AgregarCorreo']),$_POST['addTC'],$_POST['Cédula']);
							echo"
							<script type='text/javascript'>
							Swal.fire({
								title: '¡MODIFICACIÓN EXITOSA!',
                    ...
                    if($_POST['EliminarCorreo'] != ""){
                        $c->delCor(strtoupper($_POST['EliminarCorreo']),$_POST['Cédula']);
                    
                    }
            
                    if($_POST['AgregarTeléfono'] != ""){
						if($t->valTel(strtoupper($_POST['AgregarTeléfono']))==1){
							echo"
							<script type='text/javascript'>
							Swal.fire({
								title: '¡ERROR!',
								html: 'Este número de teléfono ya esta registrado y no puede ser duplicado',
								icon: 'error',
								confirmButtonText: 'VOLVER',
								allowOutsideClick: false,
								allowEscapeKey: false,
						...
						$t->addTel($_POST['AgregarTeléfono'],$_POST['addTT'],$_POST['Cédula']);
						echo"
						<script type='text/javascript'>
						Swal.fire({
							title: '¡MODIFICACIÓN EXITOSA!',
							html: 'Número de teléfono agregado',
							icon: 'info',
							confirmButtonText: 'VOLVER',
							allowOutsideClick: false,
							allowEscapeKey: false,
                    ...
                    if($_POST['EliminarTeléfono'] != ""){
                        $t->delTel($_POST['EliminarTeléfono'],$_POST['Cédula']);
