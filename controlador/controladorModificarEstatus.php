...
<!-- Page content -->
	<section class="full-box page-content">
		...
		<?php
			...
			require_once('../modelo/claseEstatus.php');
			$idDocumento = $_GET['iddoc'];
			$Estatus = $_GET['estatus'];

			$e = new estatus($Estatus,'');

			$e ->modificarEstatus($idDocumento); 

			?>
					<script>
						Swal.fire({
							title: ' ¡MODIFICACION EXITOSA! ',
							html: '¡Se ha modificado el estatus exitosamente!',
							icon: 'info',
						...
						.then(resultado=>{
							if(resultado.value){
								window.location.href='../vista/vistaEstatusSolicitud.php';
							}
						}) ;   
					</script>                              
