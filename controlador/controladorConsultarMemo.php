	...
	<section class="full-box nav-lateral">
		...
		<section class="full-box page-content">
			...
			<div class="full-box page-header">
				<h3 class="text-left">
					<i class="fas fa-plus fa-fw"></i> &nbsp; CONSULTAR MEMO
				</h3>
				<p class="text-justify">
					Se podrán consultar los datos registrados en los memos existentes.
				</p>
			</div>
			...
			<div class="container-fluid">
				...
				//echo "el numero del memo es: " . $_POST['ID'] . "<br>";

				//INCLUIMOS A LA CLASE MEMO
				require_once('../modelo/claseMemo.php');
				require_once('../modelo/claseDepartamento.php'); // INLCUIMOS CLASE DEPARTAMENTO
    			...
    			}else if(($_POST['optMemo'] == 2))
    			{
    				//echo "la fecha es:". $_POST['fecha'];

					$fecha = strtoupper($_POST['fecha']); // en mayuscula
					
					//echo $fecha;
					$memo= new memo(null,$fecha,null,null,null,null,null,null,null,null,$de); // instanciamos
					
					$result=$memo->consultarFechaMemo(); // hacemos el llamado al método


    			}else if( ($_POST['optMemo'] == 3))
    			{
    				//echo "la fecha es:". $_POST['fecha'];
					...
					$memo= new memo(null,null,null,null,null,null,null,null,$asunto,null,$de); // instanciamos

					$result=$memo->consultarAsuntoMemo(); // hacemos el llamado al método

    			}else if(($_POST['optMemo'] == 4))
    			{
    				//CONSULTAMOS SI EL DEPARTAMENTO AL QUE SE QUIERE CONSULTAR EXISTE EXISTE
					
					$para = $_POST['departamento'];

					//echo $para;

					//$departamento= new departamento(null,$para); //instanciamos un objeto de la clase
					//$row=$departamento->consultarNombreDepartamento(); // hacemos el llamado al método

					$memo= new memo(null,null,null,null,null,null,null,$para,null,null,$de); // instanciamos
					$result=$memo->consultarParaMemo(); // hacemos el llamado al método
					
    				

    			}else{

					$memo= new memo(null,null,null,null,null,null,null,null,null,null,$de); // instanciamos
					
					$result=$memo->consultarMemoDepartamento(); // hacemos el llamado al método
				...
				if(pg_num_rows($result)>0){
            		?>
                    	<h3 align='center'>INFORMACIÓN DE MEMOS</h3><br><br>
           				<div class='container-fluid'>
            			<table class='table table-dark table-sm'>
                		<thead>
                    		<tr class='text-center roboto-medium'>
                    			<td align='center' width=90><h5> CORRELATIVO MEMO</h5></td>
                    			<td align='center' width=100><h5> FECHA </h5></td>
                    			<td align='center' width=150><h5> PARA </h5></td>
                        		<td align='center' width=150><h5> ASUNTO </h5></td>
					...
					$persona = $_SESSION['nombre_persona'];
           			...
           			while($row=pg_fetch_array($result)){
                		echo "<tr><td align='center' width=90>"."<h6>".$row["idmemo"]."</h6></td>";
                		echo "<td align='center' width=100>"."<h6>".$row["fechad"]."</h6></td>";
                		echo "<td align='center' width=150>"."<h6>".$row["nombred"]."</h6></td>";
						echo "<td align='center' width=150>"."<h6>".$row["asunto"]."</h6></td>";
					
								
						echo"<td aling='center' width=100> <a href='./controladorReportesMemo.php?codigo=".$row["idmemo"].'&opt='.$de.'&persona='.$persona."' target='_blank' rel='snoopener noreferrer'>
					...
					//echo "<br>No se encontro el dato ingresado o no fue generado en este departamento";
					echo"
						<script>
							Swal.fire({
								title: 'ERROR',
								html: '<b>No se encontro el dato ingresado, consulte de nuevo<b>', 
        						icon: 'error',
