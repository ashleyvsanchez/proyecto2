		...
		<section class="full-box page-content">
			...
			<?php
				...
				if(($_POST['fecha'] !='') && ($_POST['optFiltro'] == 1))
    			{
					...
					$resultado=$correspondencia ->consultaFechaEnvio(); //llamamos al metodo
    			...
    			}else if(($_POST['asunto'] !='') && ($_POST['optFiltro'] == 2))
    			{
					...
					$resultado = $correspondencia ->consultarAsunto(); //llamamos al metodo


    			}else {
    				//CONSULTAMOS SI EL DEPARTAMENTO AL QUE SE QUIERE CONSULTAR EXISTE EXISTE
					
					$receptor = strtoupper($_POST['departamento']);
				
					$departamento= new departamento(null,$receptor); //instanciamos un objeto de la clase
					$row=$departamento->consultarNombreDepartamento(); // hacemos el llamado al método

					if($row == true){
						$correspondencia = new correspondencia('','',$receptor,'','','','',''); //instanciamos
						$resultado = $correspondencia ->consultarReceptor(); //llamamos al metodo
						...
						echo"
    						...
    						.then(resultado => {
        						if (resultado.value) {
        							window.location.href='../vista/vistaCorrespondenciaEnviada.php';
        						}
    						});
