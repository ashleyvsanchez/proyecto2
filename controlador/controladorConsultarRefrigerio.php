...
<head>
	...
	<title>CONSULTAR SOLICITUD DE REFRIGERIO</title>
...
<body>
	...
	<section class="full-box nav-lateral">
        ...
        <div class="full-box page-header">
				<h3 class="text-left">
					<i class="fas fa-plus fa-fw"></i> &nbsp; CONSULTAR SOLICITUD DE REFRIGERIO
				</h3>
				<p class="text-justify">
					Se podrán consultar los datos registrados en las solicitudes de refrigerio existentes.
				</p>
			</div>
            ...
            <div class='container-fluid'>
            	<table class='table table-dark table-sm'>
                    ...
                    <tr class='text-center roboto-medium'>
                    	...
                    	<td align='center' width=150><h5> DEPENDENCIA SOLICITANTE </h5></td>

						<td align='center' width=150><h5> CURSO O EVENTO </h5></td>
			...
			<div class="container-fluid">
				...
				//INCLUIMOS A LA CLASE REFRIGERIO
				...
				if((isset($_POST['ID'])) && ($_POST['optRefrigerio'] == 1))
				{
					$IDR = $_POST['ID'];
					...
					if(pg_num_rows($result)>0){
						...
						.then(resultado => {
						if(resultado.value) {
						window.location.href='../vista/vistaConsultarRefrigerio.php';
						}
						});
    			...
    			}else if((isset($_POST['fecha'])) && ($_POST['optRefrigerio'] == 2))
    			{
    				

    				$fecha = $_POST['fecha'];

					$refrigerio= new refrigerio(null,$fecha,null,null,null,null,null); // instanciamos
					
					$result=$refrigerio->consultarFechaRefrigerio($depid); // hacemos el llamado al método
					...
					if(pg_num_rows($result)>0){
						...
						<script type='text/javascript'>
						Swal.fire({
						title: '¡NO HAY DATOS QUE COINCIDAN CON ESTE FECHA!',
    			...
    			else if((isset($_POST['cursoevento'])) && ($_POST['optRefrigerio'] == 3))
    			{
    				

					$cursoevento = $_POST['cursoevento']; //acomodar a solo mayusculas
					
					$refrigerio= new refrigerio(null,null,null,null,null,null,null,null,$cursoevento); // instanciamos
					
					$result=$refrigerio->consultarCursoEventoRefrigerio($depid); // hacemos el llamado al método
					...
					if(pg_num_rows($result)>0){
						...
						<script type='text/javascript'>
						Swal.fire({
						title: '¡NO HAY DATOS QUE COINCIDAN CON ESTE CURSO O EVENTO!',
						html: 'Intente con otro curso o evento',
						icon: 'error',
						confirmButtonText: 'VOLVER',
						allowOutsideClick: false,
						allowEscapeKey: false,
                    ...
                    $refrigerio= new refrigerio(null,null,null,null,null,null,null); // instanciamos
					
                    $result=$refrigerio->consultarRefrigerio($depid); // hacemos el llamado al método
					...
					if(pg_num_rows($result)>0){
						...
						Swal.fire({
						title: '¡NO HAY DATOS!',
						html: 'Intente crear una solicitud de refrigerio',
						icon: 'error',
						confirmButtonText: 'VOLVER',
						allowOutsideClick: false,
						allowEscapeKey: false,
    			...
    			if(pg_num_rows($result)>0){
           			...
           			while($row=pg_fetch_array($result)){
                		echo "<tr><td align='center' width=90>"."<h6>".$row["idr"]."</h6></td>";
                		echo "<td align='center' width=100>"."<h6>".$row["fechad"]."</h6></td>";
						echo "<td align='center' width=150>"."<h6>".$row["nombred"]."</h6></td>";
						echo "<td align='center' width=150>"."<h6>".$row["cursoevento"]."</h6></td>";

						echo"<td align='center' width=100> <a href='./controladorReportesRefrigerio.php?codigo=".$row["idr"]."&depid=".$depid."' target='_blank' rel='noopener noreferrer'>
